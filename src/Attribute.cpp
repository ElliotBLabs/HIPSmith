...
Attribute::Attribute(string name, int prob) : name(name), prob(prob) {}

BooleanAttribute::BooleanAttribute(string name, int prob)
    : Attribute(name, prob) {}

string BooleanAttribute::make_random() {
...
MultiChoiceAttribute::MultiChoiceAttribute(string name, int prob,
                                           vector<string> arguments)
    : Attribute(name, prob), choices(arguments) {}

string MultiChoiceAttribute::make_random() {
  if (rnd_flipcoin(prob))
...
string AlignedAttribute::make_random() {
  if (rnd_flipcoin(prob)) {
    ostringstream oss;
...
string SectionAttribute::make_random() {
  if (rnd_flipcoin(prob)) {
    ostringstream oss;
    oss << rnd_upto(10);
