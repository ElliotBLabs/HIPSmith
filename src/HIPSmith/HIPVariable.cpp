...
void HIPVariable::ParseUnusedVars() {
      ...
      for (Statement* s : b->stms) {
        switch (s->eType) {
          ...
          case eArrayOp: {
            ...
            std::set<const Variable*> ctrlv(sao->ctrl_vars.begin(),
                                            sao->ctrl_vars.end());
    ...
    for (Block* b : f->blocks) {
      for (Variable* v : b->local_vars) {
        if (std::find(accesses.begin(), accesses.end(), v) == accesses.end()) {
          b->local_vars.erase(
              std::remove(b->local_vars.begin(), b->local_vars.end(), v),
              b->local_vars.end());
        }
      }
    }
...
void HIPVariable::Helpers::GetVarsFromExpr(std::set<const Variable*>* expr_vars,
  ...
  switch (expr->term_type) {
    case eVariable: {
      ...
      expr_vars->insert(ev->get_var()->get_collective());
      break;
    }
    case eFunction: {
    ...
    case eHIPExpression: {
      const HIPExpression* hipe = dynamic_cast<const HIPExpression*>(expr);
      if (hipe->GetHIPExpressionType() != HIPExpression::kVector) break;
...
void HIPVariable::Helpers::RecordVarInitialisation(
...
void HIPVariable::Helpers::RecordVar(std::set<const Variable*>* accesses,
                                     const Variable* var) {
...
void HIPVariable::Helpers::RecordMultipleVars(
    std::set<const Variable*>* accesses, std::set<const Variable*>* vars) {
